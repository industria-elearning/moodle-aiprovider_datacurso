...
/**
   ...
   */Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.init=void 0,_ajax=_interopRequireDefault(_ajax),_chartjs=_interopRequireDefault(_chartjs);_exports.init=()=>{console.log("📊 Report init");const tokensAvailable=document.getElementById("tokens-available"),tokensConsumed=document.getElementById("tokens-consumed"),tableBody=document.getElementById("consumption-table-body");let chartBar,chartPie,consumos=[];Promise.all([_ajax.default.call([{methodname:"aiprovider_datacurso_get_consumption_history",args:{}}])[0],_ajax.default.call([{methodname:"aiprovider_datacurso_get_tokens_saldo",args:{}}])[0]]).then((async _ref=>{let[historyResponse,saldoResponse]=_ref;consumos=(null==historyResponse?void 0:historyResponse.consumos)||[];const saldo=(null==saldoResponse?void 0:saldoResponse.saldo_actual)||0;tokensAvailable.textContent=saldo,tokensConsumed.textContent=consumos.reduce(((sum,c)=>sum+(c.cantidad_tokens||0)),0),renderTable(consumos,tableBody),initCharts(consumos)})).catch((err=>{console.error("❌ Error WS",err)}));const renderTable=async(data,container)=>{if(container.innerHTML="",data.length)data.forEach((item=>{const row=document.createElement("tr");row.innerHTML="\n                <td>".concat(item.id_consumo,"</td>\n                <td>").concat(item.id_usuario,"</td>\n                <td>").concat(item.accion,"</td>\n                <td>").concat(item.servicio,"</td>\n                <td>").concat(item.cantidad_tokens,"</td>\n                <td>").concat(item.saldo_restante,"</td>\n                <td>").concat(item.fecha,"</td>\n            "),container.appendChild(row)}));else{const nodata=await(0,_str.get_string)("nodata","aiprovider_datacurso");container.innerHTML='<tr><td colspan="7" class="text-center">'.concat(nodata,"</td></tr>")}},initCharts=data=>{const filterBar=document.getElementById("filter-service-bar"),filterPie=document.getElementById("filter-service-pie");[...new Set(data.map((c=>c.servicio)))].forEach((s=>{filterBar.innerHTML+='<option value="'.concat(s,'">').concat(s,"</option>"),filterPie.innerHTML+='<option value="'.concat(s,'">').concat(s,"</option>")})),renderCharts(data),filterBar.addEventListener("change",(()=>renderCharts(data))),filterPie.addEventListener("change",(()=>renderCharts(data)))},renderCharts=data=>{const filterBar=document.getElementById("filter-service-bar").value,filterPie=document.getElementById("filter-service-pie").value;let filteredData=data;"ALL"===filterBar&&"ALL"===filterPie||(filteredData=data.filter((c=>!("ALL"!==filterBar&&c.servicio!==filterBar||"ALL"!==filterPie&&c.servicio!==filterPie))));const byMonth={};filteredData.forEach((c=>{const month=c.fecha.substring(0,7);byMonth[month]=(byMonth[month]||0)+c.cantidad_tokens}));const byAction={};filteredData.forEach((c=>{byAction[c.accion]=(byAction[c.accion]||0)+c.cantidad_tokens}));const ctx1=document.getElementById("chart-tokens-by-month");chartBar&&chartBar.destroy(),chartBar=new _chartjs.default(ctx1,{type:"bar",data:{labels:Object.keys(byMonth),datasets:[{label:"Tokens consumidos",data:Object.values(byMonth),backgroundColor:"#0073e6"}]}});const ctx2=document.getElementById("chart-actions");chartPie&&chartPie.destroy(),chartPie=new _chartjs.default(ctx2,{type:"pie",data:{labels:Object.keys(byAction),datasets:[{data:Object.values(byAction),backgroundColor:["#36A2EB","#FF6384","#FFCE56","#4BC0C0"]}]}})}}}));
